#2nd file in a 2 script pipeline for a complete RNA-seq cleaning of paired-end files. Picks up where auto clean left off and automates 2 bowties based on LSU and SSU indexes, removes identified sequences, performs hisat2, and uses BTI specific Fei scripts to create a normalized RPKM file.
...
#creates a list of files to be read from the prinseq folder, specifies good output and single side of paired duo exclusively
import glob, os
os.chdir(".")
fastq_good = []
for file in glob.glob("prinseq/*R*good*.fastq"):
    fastq_good.append(file)
#Housekeeping prints
#print(fastq_good[1])
#print(fastq_good[1][8:-15])
#print(fastq_good[1][8:-16])

#applies bowtie to prinseq output list formed above, long sequence units trimmed first
bowtie_name = []
...
#applies bowtie to prinseq output list formed above, short sequence units trimmed second
for name in fastq_good:
  ...
  subprocess.call(bow, shell=True)
print(len(bowtie_name))
#print(bowtie_name[1][7:-3])
...
def trim_fastq(bowtie):
  print("Trimming " + bowtie[7:]  + " based on " + bowtie[7:])
  ...
  #clear variable for memory efficiency
  header_set2 = None
  records = []
...
#read all filenames from the bowtie_output/ folder that are part of a paired end duo
fastq_good = []
for file in glob.glob("bowtie_output/*R*.fastq"):
    fastq_good.append(file)
print(len(fastq_good))
print(fastq_good[1][14:-6])
